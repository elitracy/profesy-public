...
function getDependenciesFromBundledNativeModules({
  ...
  if (shouldUseExpoReactNativeFork) {
    var _targetSdkVersion$pac;

    result['react-native'] = ((_targetSdkVersion$pac = targetSdkVersion.packagesToInstallWhenEjecting) === null || _targetSdkVersion$pac === void 0 ? void 0 : _targetSdkVersion$pac['react-native']) || `https://github.com/expo/react-native/archive/${targetSdkVersion.expoReactNativeTag}.tar.gz`;
  } else {
...
async function upgradeAsync({
  ...
  const expoPackageToInstall = _getenv().default.boolish('EXPO_BETA', false) ? `expo@next` : `expo@^${targetSdkVersionString}`;
  const exactExpoPackageVersion = await getExactInstalledModuleVersionAsync('expo', projectRoot);
  const isExpoPackageVersionPrerelease = exactExpoPackageVersion && _semver().default.prerelease(exactExpoPackageVersion) !== null; // Skip installing the Expo package again if it's already installed. @satya164
