...
async function launchAppAsync(appName, launchArgs) {
    ...
    if (os_1.default.platform() === 'darwin' && !Array.isArray(appName)) {
        ...
        const appPath = (_a = glob_1.sync('Contents/MacOS/*', { cwd: appDirectory.trim(), absolute: true })) === null || _a === void 0 ? void 0 : _a[0];
        if (!appPath) {
            throw new Error(`Cannot find application path from ${appDirectory}Contents/MacOS`);
        }
        closeJsInspector();
        openingChildProcess = child_process_1.spawn(appPath, launchArgs, { stdio: 'ignore' });
        return;
    }
