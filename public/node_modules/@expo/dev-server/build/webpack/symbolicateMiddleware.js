...
function createSymbolicateMiddleware({ projectRoot, logger, compiler, }) {
    const getFilenameFromUrl = getFileAsync_1.createGetFileNameFromUrl(compiler);
    const symbolicate = new Symbolicator_1.Symbolicator({
        ...
        async getFileAsync(props) {
            ...
            return getFileAsync_1.getFileFromCompilerAsync(compiler, { fileName, platform: props.platform });
        },
        async getSourceMapAsync(props) {
            var _a, _b;
            const fileName = getFilenameFromUrl(props);
            const fallbackSourceMapFilename = `${fileName}.map`;
            const bundle = await getFileAsync_1.getFileFromCompilerAsync(compiler, {
                fileName,
            ...
            for (const file of [sourceMapFilename, fallbackSourceMapFilename]) {
                try {
                    return await getFileAsync_1.getFileFromCompilerAsync(compiler, {
                        fileName: file,
                        platform: props.platform,
                    });
                }
    ...
    return async function (req, res) {
        ...
        try {
            ...
            const platform = (_b = (_a = getFileAsync_1.getPlatformFromRequest(req)) !== null && _a !== void 0 ? _a : Symbolicator_1.Symbolicator.inferPlatformFromStack(stack)) !== null && _b !== void 0 ? _b : 'web';
            if (!platform) {
                return res
                    .writeHead(400)
